...
function add_to_list(ctx,name, list){
    if (list.length > 4)
        ctx.reply('Ya esta llena la lista, dormiste.')
    list.push(name)
}
...
bot_listas.command('toy', (ctx) => {
    ...
    if (!listas[ctx.chat.id].includes(ctx.from.first_name)){    
        ...
        ctx.reply('Adentro '+ctx.from.first_name)
    }else{
        ctx.reply('Ya estas en la lista pa.')
    }
    })
...
bot_listas.command('salir', (ctx) => {
    ...
    if (index > -1) {
        ...
        ctx.reply('Sos tremendo gil, chau.');
    }else{
        ctx.reply('De donde queres salir vos, banana.');
    }
})
